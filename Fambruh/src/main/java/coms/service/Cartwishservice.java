...
public class Cartwishservice {
    ...
    public void addToCart(Product product, int quantity, String username) {
        User user = userRepository.findByUsername(username);
        ProductQuantity productQuantity = new ProductQuantity(); // Create a new ProductQuantity instance
        productQuantity.setProduct(product); // Set the product for the ProductQuantity
        productQuantity.setQuantity(quantity); // Set the quantity for the ProductQuantity
        CartItem cartItem = new CartItem(user, product, null, null, productQuantity,0);
